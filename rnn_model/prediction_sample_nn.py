...
if __name__ == "__main__":
    ...
    for lhs_sb, rel_sb, rhs_sb in test_iter: # one batch
        lhs_s, lhs_mask_s, lhs_emb_s, rel_s, rhs_s = \
                batch.prepare_vs_nn(lhs_sb, rel_sb, rhs_sb, chardict, lhs_dict, rel_dict, rhs_dict, n_chars=n_char) # TODO change model
        test_mean_rank = m.rank_right(lhs_s, lhs_mask_s, lhs_emb_s, rel_s, rhs_s)
        mean_rank += test_mean_rank

        X_vis = np.concatenate((X_vis, m.pred_rel(lhs_s, lhs_mask_s, lhs_emb_s, rhs_s)), axis=0)
        Y_vis += [(str(ii) + '\n') for ii in rel_s]
